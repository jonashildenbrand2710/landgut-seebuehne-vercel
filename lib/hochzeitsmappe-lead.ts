...
export async function submitHochzeitsmappeLeadToActiveCampaign(lead: HochzeitsmappeDeliveryLead) {
  ...
  const alreadySubscribed = flowConfig.listId
    ? await isActiveCampaignContactSubscribedToList(activeCampaign, contactId, flowConfig.listId)
    : false;
  ...
  // New contacts enter through the list-subscription trigger. Existing list
  // members enter directly so a renewed request also sends the current access link.
